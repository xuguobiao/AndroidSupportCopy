...
public class FixedTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private List<TabItemInterface> mTabItems = new ArrayList<>();
     ...
     * id
     */
    private static final int ID_ITEM_LL_TAP = 2009000;
    ...
    public void setTabData(List<TabItem> tabItems) {
        if (tabItems == null || tabItems.size() == 0) {
            throw new IllegalStateException("tabItems can not be NULL or EMPTY !");
        }

        this.mTabItems.clear();
        this.mTabItems.addAll(tabItems);
    ...
    private View createTabView(int iconGravity) {
        ...
        titleTextView.setId(ID_ITEM_TV_TAB_TITLE);
    ...
    private void addTab(final int position, View tabView) {
        ...
        tv_tab_title.setText(mTabItems.get(position).getTabTitle());
        ImageView iv_tab_icon = (ImageView) tabView.findViewById(ID_ITEM_IV_TAB_ICON);
        updateTabIcon(iv_tab_icon, mTabItems.get(position).getTabUnselectedIcon());
...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
            ...
            if (mIconVisible) {
                ...
                TabItemInterface tabEntity = mTabItems.get(i);
                updateTabIcon(iv_tab_icon, i == mCurrentTab ? tabEntity.getTabSelectedIcon() : tabEntity.getTabUnselectedIcon());
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            TextView tab_title = (TextView) tabView.findViewById(ID_ITEM_TV_TAB_TITLE);
            tab_title.setTextColor(isSelect ? mTextSelectColor : mTextUnselectColor);
            ImageView iv_tab_icon = (ImageView) tabView.findViewById(ID_ITEM_IV_TAB_ICON);
            TabItemInterface tabEntity = mTabItems.get(i);
            updateTabIcon(iv_tab_icon, isSelect ? tabEntity.getTabSelectedIcon() : tabEntity.getTabUnselectedIcon());
    ...
    private void updateTabIcon(ImageView icon, int drawableResId) {
        if (icon != null) {
            if (drawableResId == INVALID_RES_ID) {
    ...
    public static class TabItem implements TabItemInterface {
        public String title;
        public int selectedIcon;
        public int unSelectedIcon;

        public TabItem(String title) {
            this(title, INVALID_RES_ID, INVALID_RES_ID);
        }
